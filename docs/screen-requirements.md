...
| SC.7 | スケジュール一覧 | 閲覧 | しおりの特定日のスケジュール群の確認 |
| SC.8 | スケジュール詳細 | 閲覧 | 個別スケジュールの確認 |
| SC.9 | スケジュール作成 | 編集 | 新しいスケジュールの登録 |
| SC.10 | スケジュール編集 | 編集 | 既存スケジュールの更新 |
...
特定のしおりの基本情報と、その旅の全体像を確認する画面である。日々のスケジュールそのものは別画面で扱い、ここでは旅全体の概観に集中する。

### 出力

出力する情報は、しおりの属性、参加メンバーの構成、そして地点情報を持つスケジュールの地図表示を含む。参加メンバーは複数のユーザーから成り、各メンバーは表示名とアイコン画像という属性を持つ。地図表示には旅程全体で地点情報を持つスケジュールが配置される。
...
しおりの中の特定日のスケジュール群を確認・操作する画面で、日々の旅程を把握するための主要画面である。

### 出力

画面は対象しおりの識別情報、選択中の日付、その日のスケジュール群、そして地点情報を持つスケジュールの地図表示を出力する。スケジュールの一覧の各エントリは、時刻、タイトル、カテゴリといった基本情報に加え、地点情報を持つ場合は地点名と写真も含む。地点情報は任意のため、すべてのスケジュールが同じ情報量を持つわけではない。地図表示はその日のうち地点情報を持つスケジュールを配置する。

- 対象しおりのタイトル
- 選択中の日付
- その日に登録されているスケジュールの一覧。各スケジュールについて以下を含む。
  - 開始時刻と終了時刻
  - タイトル
  - カテゴリ
  - 地点名（地点情報を持つ場合）
...
実行できるアクションは表示日付の切替、個別スケジュールの確認、新規スケジュールの作成の3つである。

- 表示する日付の選択
- 特定のスケジュールを指定する（SC.8 スケジュール詳細へ遷移）
- 新規スケジュールの作成を始める（SC.9 スケジュール作成へ遷移）

## SC.8 スケジュール詳細

特定のスケジュールの詳細情報を確認する画面である。

### 出力

スケジュールの全属性を出力するが、地点情報を持たないスケジュールでは地点情報部分が表示されない。地点情報を持つスケジュールでは、地点名、地図リンク、写真の3要素が組として含まれる。
...
- スケジュールの編集を開始する（SC.10 スケジュール編集へ遷移）
- スケジュールの削除
- 外部地図アプリへ遷移（地点情報を持つ場合のみ）

## SC.9 スケジュール作成

新しいスケジュールを登録するフォーム画面である。
...
- 外部API検索による地点の検索と選択
- 入力内容を確定して新規スケジュールを登録する（成功時は SC.7 スケジュール一覧へ戻る）
- 入力を破棄して戻る

## SC.10 スケジュール編集

既存のスケジュールの情報を更新する画面である。

### 出力

画面は編集対象のスケジュールの現在の全属性を出力する。これは編集前の値であり、ユーザーが書き換えなかった項目はこの値のまま維持される。

- 編集対象のスケジュールの現在のタイトル
- 編集対象のスケジュールの現在の日付
- 編集対象のスケジュールの現在の開始時刻と終了時刻
- 編集対象のスケジュールの現在のカテゴリ
- 編集対象のスケジュールの現在のメモ
- 編集対象のスケジュールの現在の地点情報（地点情報を持つ場合）。以下を含む。
  - 地点名
  - 地図リンク
  - 写真
...
- 外部API検索による地点の差し替え
- 既存の地点情報の解除
- 変更を保存する（成功時は SC.8 スケジュール詳細へ戻る）
- 変更を破棄して SC.8 スケジュール詳細へ戻る
...
flowchart LR
  ...
  shioriDetail --> scheduleList[スケジュール一覧]
...
ホームを起点として、しおり一覧、しおり詳細、しおり作成、設定の4方向に分岐する。しおり詳細からはしおり編集とスケジュール一覧へ進める。スケジュール一覧からはスケジュール詳細、スケジュール作成へと進め、スケジュール詳細からはスケジュール編集へと進める。編集系の画面は、対応する閲覧画面へ戻る形で完結する。
