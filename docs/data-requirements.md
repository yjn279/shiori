...
```mermaid
erDiagram
  user ||--o{ membership : "参加"
  shiori ||--|{ membership : "メンバー"
  shiori ||--o{ schedule : "含む"
```

しおりは1人以上のメンバー(最低1人は作成者)を持つ。予定はしおりに従属する。予定は外部地図APIまたはPlace APIへの参照を任意で保持できるが、参照先の地点情報そのものは本サービスでは保持しない。地点を持たない予定(移動中の休憩、宿への到着、まだ場所が決まっていない予定など)も自然に表現できる。
...
旅行ごとに作成される、本サービスの中心エンティティ。予定とメンバーシップを子として束ねる。
...
ユーザーとしおりの組み合わせは一意である。各しおりは creator の役割を持つメンバーシップを正確に1件持つ。参加日時は共通属性の作成日時で代替するため、独立した属性は持たない。

### 予定

しおりの中の特定の日付に紐づく単位情報を表す。時刻は開始時刻と終了時刻で表現し、それぞれ任意とする。時刻を持たない予定は時刻情報なしの予定として扱う。地点が特定される予定は、外部地図APIまたはPlace APIへの参照を保持する。
...
| 作成者ユーザー | 参照 | 必須 | 予定を作成したユーザー |

外部API識別子は、予定が地点に紐づく場合にのみ設定される。地点名、地図リンク、緯度、経度、写真などその他の地点関連情報は、本サービスではエンティティとして保持しない。これらは表示時に外部APIから取得する。識別子だけを保持し、他の情報を都度取得する方針は、データの単一情報源を外部APIに集約し、地点情報の更新を本サービス側で追従する必要をなくすことを意図する。
